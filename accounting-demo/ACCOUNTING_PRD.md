...
4. **密码输入框**：
   ...
   - 显示/隐藏切换按钮
5. **用户协议复选框**：
   - 文本：我已阅读并同意《服务协议》和《隐私政策》
   - 协议文本为可点击链接，点击弹出模态框展示完整内容
6. **登录按钮**：
   - 文本：登录 / 登录中...（加载状态）
   - 按钮状态：禁用时显示灰色，启用时显示渐变主题色
7. **辅助操作链接**：
   - 注册账号（跳转到注册页面）
   - 忘记密码？（跳转到忘记密码流程）
8. **游客模式入口**：
   - 文本：先体验一下 >
   - 点击后弹出确认对话框，确认后以游客身份进入首页
...
**成功登录流程：**
1. 验证通过后显示"登录中..."状态
2. API调用成功：保存token和用户信息到本地存储
3. 显示成功提示："登录成功"
4. 1秒后自动跳转到账单首页（tabBar第一个页面）
5. 页面跳转：`uni.switchTab({ url: '/pages/tabBar/bill/index' })`
...
3. 显示成功提示："注册成功"
4. 自动填充登录表单（账号）
5. 跳转到登录页面或直接登录（根据设计决策）
6. 建议流程：注册成功后自动登录并跳转首页
...
**流程说明：**
1. 输入注册的手机号或邮箱 → 点击"发送重置链接"
2. 系统向注册邮箱发送包含重置链接的邮件
3. 用户点击邮件中的链接 → 跳转到重置密码页面
4. 输入新密码并确认 → 提交重置
5. 重置成功后跳转到登录页面
...
2. 传输加密：HTTPS + 请求体加密
3. 密码错误次数限制：5次锁定30分钟
4. 密码历史记录：防止重复使用最近3次密码

**设备识别：**
1. 记录登录设备信息
2. 新设备登录需要邮箱验证（发送验证邮件）
3. 支持查看登录设备列表

**会话管理：**
1. Access Token有效期：7天
2. Refresh Token有效期：30天
3. 支持多设备同时登录
4. 支持手动登出所有设备
