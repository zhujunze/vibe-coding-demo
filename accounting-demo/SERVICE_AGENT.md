...
### 3.6 接口文档同步与维护
*   **同步原则**: 遵循“文档即代码”原则。开发人员在完成 API 接口编码后，必须立即完善 OpenAPI 注解，确保生成的文档与代码逻辑保持严格同步。
*   **文档落地**:
    *   为便于版本追踪与协作，需在项目根目录下维护一份最新的离线接口文档。
    *   **存放位置**: 项目根目录 (`/`)。
    *   **命名规则**: `openapi-spec.yaml` (始终代表当前分支最新状态)。
    *   **格式要求**: 必须符合 OpenAPI 3.0 规范 (YAML/JSON)。
*   **内容完整性**:
    *   文档必须包含：接口路径 (Path)、请求方法 (Method)、请求参数 (Query/Body/Header，含类型、必填项、示例)、响应结构 (Schema)、以及完整的错误码 (Error Codes) 说明。
*   **维护责任**:
    *   接口开发人员是文档的第一责任人。
    *   任何接口逻辑变更（如新增字段、修改校验规则）必须同步更新对应的注解及根目录下的规范文件。
*   **审查标准 (Code Review)**:
    *   **注解检查**: 检查 `@Operation`, `@Schema`, `@ApiResponse` 是否完整、准确。
    *   **文件同步**: 凡涉及 API 变更的 PR，必须包含 `openapi-spec.yaml` 的更新。
    *   **一致性**: 若发现文档描述与代码实现不符，Review 不予通过。
